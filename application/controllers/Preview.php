        ...
        for ($i=1; $i<=$rows; $i++) {
            ...
            for ($j=1; $j<=$cols; $j++) {
                if ($items[$i][$j]['type']=='q') {
                    ...
                    $row .= '<div class="sk_inner_box"><input autocomplete="off" type="text" '.$val.' data_q_type="0" data_num_colofrow="'.$i.'_'.$j.'" value="" name="skip_counting[]" class="form-control input-box ans_input  rsskpinpt'.$i.'_'.$j.'"  style="min-width:45px; max-width:45px">';
...
//        $left_memorize_p_one = array_map('strtolower', $left_memorize_p_one);
    ...
    public function preview_demo($question_item, $question_id){
        ...
        $data['question_item']     = $question_item;
        $data['question_info_ind'] = $data['question_info'];
        $data['demoquestions']     = $data['question_info_s'][0]['demoquestions'];
        $data['questionImage']     = $data['question_info_s'][0]['questionImage'];
