...
class Student extends CI_Controller
{
    ...
    private function openModuleByTutorialBased($modle_id, $question_order_id)
    {
        ...
        //echo "<pre>";print_r($data['tutorial_ans_info']);die();
        ...
        } elseif($data['question_info_s'][0]['question_type'] == 24){

            $_SESSION['q_order_2'] = $this->uri->segment('3');
            $data['maincontent'] = $this->load->view('students/question_module_type_tutorial/ans_demo', $data, true);
    ...
    public function get_tutor_tutorial_module($modle_id, $question_order_id, $is_every_study = 0)
    {
        ...
        // echo '<pre>';print_r($get_student_ans_info);die();
    ...
    public function save_student_answer($module_id)
    {
        ...
        if ($ans_array != "") {
            ...
            if ($get_module_info[0]['isSMS'] == 1) {
                ...
                if (!empty($courseName)) {
                    $new_course_name = preg_split('#<p([^>])*>#',$courseName[0]['courseName']);
                    $new_course_name = array_filter($new_course_name);
                    if(isset($new_course_name[0])){
                        $course = $new_course_name[0];
                    }else if(isset($new_course_name[1])){
    ...
    public function st_answer_matching_demo(){
    ...
    public function studentsModuleByQStudy()
    {
        ...
        if (isset($tutorInfo[0]['user_type']) && $tutorInfo[0]['user_type'] == 7) { //q-study
            ...
            // echo '<pre>';print_r($conditions);die;
            // $all_module = $this->ModuleModel->allModule(array_filter($conditions));
            $all_module = $this->ModuleModel->allModule(array_filter($all_module), $studentGrade_country[0]['country_id']);
            // echo '<pre>';print_r($all_module);die;
            // echo $this->db->last_query();die();
        }
        // echo "<pre>";print_r($all_module);die();
    ...
    public function show_repetition_module($allModule)
    {
        ...
        $chapters = [];
        $st_chapters = [];
        // echo "<pre>";print_r($allModule);die();
        if ($allModule) {
